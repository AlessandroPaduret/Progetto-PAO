...
RecurrentEvent::RecurrentEvent(RecurrenceStrategy<Event> recurrenceStrategy,
                               Event templateEvent)
...
RecurrentEvent::getSchedulable(const TimePoint from, const TimePoint to) const {
  ...
  // Scorri le date generate dalla strategia di ricorrenza nell'intervallo
  // specificato
  ...
  for (const TimePoint tp : dates) {
    ...
    // Se esiste una modifica specifica, usala; altrimenti, crea un'istanza
    // standard basata sull'evento template
    if (specificItem) {
...
void RecurrentEvent::addModification(TimePoint tp,
                                     std::unique_ptr<Event> modifiedEvent) {
