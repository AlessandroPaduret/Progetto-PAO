...
- `app/` — Qt6 desktop app **standalone** (single-window, deliverable PAO; native build, NOT containerized; MVC: `CalendarController` + views):
  ...
  - `app/src/views/` — `MainWindow` (QStackedWidget con 3 pagine: 0=settimana, 1=elenco, 2=dettaglio; toolbar con Nuovo/Modifica/Elimina/Salva/Carica via QFileDialog, mai path cablati), `WeekView` (griglia settimanale QPainter, da client/; lavora su `events::Occurrence`, durata zero = chip da 18px; **eventi sovrapposti affiancati a colonne** come Google Calendar; **drag&drop**: tenere premuto un'occorrenza e rilasciarla su una nuova cella emette `activityMoved` -> `CalendarController::moveActivity` -> `Activity::moveTo`; per i ricorrenti lo spostamento NON trasla la fine della serie (resta la stessa; se il nuovo inizio supererebbe la fine, la fine viene portata al nuovo inizio per non restare antecedente), le eccezioni vengono traslate; segnale `activityEditRequested` dal doppio clic per modificare l'attivita' intera conservando il tipo; menu contestuale: per i ricorrenti "Modifica" (serie) + "Modifica istanza" (singola occorrenza), per gli altri "Modifica" dell'attivita'. `ActivityFormPage::onSave` valida che la scadenza della serie sia successiva all'inizio), `ActivityListPage` (tabella + ricerca live per titolo via `Calendar::search`), `ActivityDetailPage` (campi specifici per tipo via `FieldsVisitor`), `ActivityFormPage` (form per tipo in QStackedWidget: Evento/Ricorrente/Scadenza/Promemoria; modi Create/EditActivity/EditOccurrence), `ActivityFormDialog` (pannello ridotto DENTRO la MainWindow — widget figlio, quindi non puo' uscire dai bordi: dimensioni boundedTo(host), ricentrato in `MainWindow::resizeEvent`, intestazione interna senza pulsante di chiusura, si chiude con Salva/Annulla; il form e' in una QScrollArea). `ActivityFormPage`: campi propri per pannello in QGridLayout (etichetta a sinistra, box di input affiancata), titolo/data conservati al cambio tipo (syncCommonFields — niente widget condivisi tra i pannelli dello stack). `ActivityViewHelpers` (`typeLabel/summaryLabel/recurrenceRuleLabel/durationLabel` — etichette via Visitor, solo display).
