...
attivita' personali -- impegni, eventi, e incontri -- interamente
scritta in C++ con interfaccia grafica realizzata in Qt. Gli
utenti possono creare, modificare, eliminare, cercare e visualizzare i diversi
tipi di attività. Le \textbf{attività} possono essere semplici, avere una \textbf{checkbox}
oppure, nel caso dei \textbf{meeting}, avere dettagli come location e partecipanti. 

L'architettura del progetto separa nettamente il modello logico dall'interfaccia
grafica:

\begin{itemize}[nosep]
  \item \textbf{modello logico} (\texttt{model/}): una libreria con elementi dello standard C++23
  ...
  \item \textbf{persistenza} (\texttt{model/persistence/}): un modulo separato
    (che usa \texttt{QJsonObject}) per serializzare e ricaricare le attività su file
...
esclusivamente dai \textbf{builder} \texttt{ActivityBuilder}/
\texttt{TaskBuilder}/\texttt{MeetingBuilder} (pattern \textbf{Builder}),
affiancati da \texttt{GeneratorBuilder} per le regole di ricorrenza.
...
\begin{enumerate}[nosep]
  \item \textbf{settimana}: griglia settimanale stile Google Calendar disegnata
    ...
    dati dell'attivita' (per un ricorrente l'inizio della serie si sposta ma la
    \emph{scadenza resta quella}; se la scadenza diventerebbe antecedente
    all'inizio viene portata al nuovo inizio); doppio clic su una cella vuota
    avvia la creazione precompilata;
...
checkbox, con la validazione che la fine sia \emph{successiva} all'inizio)
per i ricorrenti; priorita' (bassa/media/alta) e stato "evasa" per le
...
con regola e data di fine modificabili); per un ricorrente il menu contestuale
offre sia la \textbf{modifica della serie} sia la \textbf{modifica della singola
istanza}. Il pannello si chiude con i pulsanti
Salva/Annulla.
...
Refactoring (Zero-cost Abstaction) & 10h & 20h \\
\midrule
\textbf{Totale} & \textbf{48h} & \textbf{56h} \\
\bottomrule
\end{tabular}
