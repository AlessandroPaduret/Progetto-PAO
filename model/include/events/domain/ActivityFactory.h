 ...
 *        I Builder (ActivityBuilder/TaskBuilder/MeetingBuilder) sono il
 *        percorso consigliato; la factory resta per compatibilita'.
 */
class ActivityFactory {
public:
    /** @brief Crea un evento singolo (Activity con SingleGenerator)
     ...
     */
    static std::unique_ptr<Activity> createSimpleEvent(const String& title, TimePoint start, Duration duration);

    /** @brief Crea un evento ricorrente a intervallo fisso
     ...
     *  @return Un puntatore unico all'attivita' ricorrente creata
     */
    static std::unique_ptr<Activity> createRecurrentEvent(const String& title, TimePoint start, Duration duration, Duration interval);
     ...
     *  @return Un puntatore unico all'attivita' ricorrente creata
     */
    static std::unique_ptr<Activity> createSimpleWeekly(const String& title, TimePoint start, Duration duration, TimePoint end);
     ...
     *         come Activity con YearlyGenerator e durata "tutto il giorno".
     *  @param title Il titolo (es. "Mario - Compleanno")
     *  @param date La data base (mezzanotte; giorno/mese definiscono la ricorrenza)
     *  @return Un puntatore unico all'attivita' creata
     */
    static std::unique_ptr<Activity> createAnniversary(const String& title, TimePoint date);
};
