...
- `app/` — Qt6 desktop app **standalone** (single-window, deliverable PAO; native build, NOT containerized; MVC: `CalendarController` + menu + views):
  ...
  - `app/src/views/` (+ header in `app/include/views/`) — viste Qt (MVC, mai path cablati):
    ...
    - `ViewShared.h` — helper condivisi inline, solo Qt Gui (niente Widgets: incluso anche da `app_controller_tests`, che collega Qt Test + Qt Gui ma non Qt Widgets): `activityColor` (paletta stabile dall'indirizzo dell'oggetto), `localTime` (UTC->locale), `shortDayName(dayOfWeek)`.
