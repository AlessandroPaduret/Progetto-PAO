...
- `app/` — Qt6 desktop app **standalone** (single-window, deliverable PAO; native build, NOT containerized; MVC: `CalendarController` + views):
  ...
  - `app/src/views/` — viste Qt (MVC, mai path cablati):
    ...
    - `ViewShared.h` — helper condivisi inline: `activityColor` (paletta stabile dall'indirizzo dell'oggetto), `localTime` (UTC->locale), `shortDayName(dayOfWeek)`, `repolish(QWidget*)` (rivaluta le regole QSS dipendenti da una proprieta' dinamica dopo un `setProperty`, via `style()->unpolish/polish`).
